...
def _validate_categories(things: Set[int], stuff: Set[int]) -> None:
    ...
    if len(things) == 0:
        raise ValueError("Expected argument `things` to be a non-empty `Set[int]`")

    if not _is_set_int(stuff):
        raise ValueError("Expected argument `stuff` to be of type `Set[int]`")
    if len(stuff) == 0:
        raise ValueError("Expected argument `stuff` to be a non-empty `Set[int]`")
...
def _validate_inputs(preds: torch.Tensor, target: torch.Tensor) -> None:
    ...
    if preds.dim() != 4 or preds.shape[1] != 2:
        raise ValueError("Expected argument `preds` to have shape [batch, 2, height, width]. Dim 1 corresponds to (category_id, instance_id)")
...
def _make_instance_ids_batch_unique(arr: torch.Tensor):
    ''' Ensure all instance ids are unique accross batches '''
    if arr.shape[0] == 1:
        return arr
    # Add the biggest instance id from previous batch
    for b in range(arr.shape[0]-1):
        arr[b+1,1] += arr[b,1].max()
    return arr
...
def _prepocess_image(
    ...
    img = _make_instance_ids_batch_unique(img)
    # Original shape: [b,(sem_id,inst_id),h,w]
    # Original shape: [b,h,w,(sem_id,inst_id)]
    img = img.moveaxis(1,-1)
    # flatten the batch*height*width dimensions
