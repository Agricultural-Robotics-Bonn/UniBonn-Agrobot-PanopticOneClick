"""
  From detectron2/projects/Panoptic-DeepLab/panoptic_deeplab/post_processing.py
"""
...
def get_instance_segmentation(
    ...
    Returns:
        ...
        A Tensor of shape [1, K, 2] where K is the number of center points.
            The order of second dim is (y, x).
    """
    if len(center_heatmap.size())==2:  # centers here already (because i parse them when using clicks as input when no cnt head is available...)
        center_points = center_heatmap
    elif len(center_heatmap.size())==3:  # regular way
...
def merge_semantic_and_instance(
    ...
    if void_label is not None and len( void_label ) > 0:
...
def get_panoptic_segmentation(
    ...
    ## commented this so it takes the cntcoords parsed when no cnt head is available
    ...
    # outlier detection
    imap = torch.zeros_like( instance )
    for u in torch.unique( instance ):
      if u == 0:
        continue
      t = torch.where( instance==u )
      if t[1].shape[0] < 10:
        continue
      mur, stdr = torch.mean( t[1].float() ), torch.std( t[1].float() )
      muc, stdc = torch.mean( t[2].float() ), torch.std( t[2].float() )
      minr, maxr = torch.max( torch.tensor( [mur-(stdr*3), 0] ) ), torch.min( torch.tensor( [mur+(stdr*3), imap.shape[1]] ) )
      minc, maxc = torch.max( torch.tensor( [muc-(stdc*3), 0] ) ), torch.min( torch.tensor( [muc+(stdc*3), imap.shape[2]] ) )
      r = torch.logical_and( t[1] >= minr, t[1] <= maxr )
      c = torch.logical_and( t[2] >= minc, t[2] <= maxc )
      v = torch.logical_and( r, c )
      imap[0,t[1][v],t[2][v]] = u
    return panoptic, center, sem_seg, imap
